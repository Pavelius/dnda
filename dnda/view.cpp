...
static void view_info(const creature& e) {
	...
	int y1 = y;
	int x1 = x;
	y += field(x, y, tw, "СЛ", e.get(Strenght));
	y += field(x, y, tw, "ИН", e.get(Intellegence));
	x += dx;
	y = y1;
	y += field(x, y, tw, "ЛВ", e.get(Dexterity));
	y += field(x, y, tw, "МД", e.get(Wisdow));
	x += dx;
	y = y1;
	y += field(x, y, tw, "ТЛ", e.get(Constitution));
	y += field(x, y, tw, "ХР", e.get(Charisma));
	x += dx + 6;
	y = y1;
