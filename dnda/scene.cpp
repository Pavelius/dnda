...
static unsigned select(aref<creature*> result, aref<creature*> source, sceneparam& sp, int range) {
	auto ps = result.data;
	auto pe = result.data + result.count;
	auto flags = sp.flags;
	auto index = sp.player.getposition();
	for(auto p : source) {
...
bool scene::apply(const sceneeffect& eff, bool run) {
	...
	if(eff.proc.cre) {
		...
		source.count = select(source, creatures, sp, los);
		if(run) {
			if((eff.flags&All) != 0) {
				for(auto& e : source)
